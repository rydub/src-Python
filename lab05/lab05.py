...
def reduce(combiner, seq):
    ...
    4
    """
    def reducer(combiner, seq):
        if len(seq) == 1:
            return seq[0]
        return combiner(seq[0], reducer(combiner, seq[1:]))
    return reducer(combiner, seq)
...
def num_songs(t):
    ...
    if is_leaf(t):
        return 1
    return sum([num_songs(n) for n in branches(t)])
...
def add_song(t, song, category):
    ...
    """
    if root(t) == category:
        return tree(category, [add_song(j, song, category) for j in branches(t)] + [tree(song)])
    return tree(root(t), [add_song(i, song, category) for i in branches(t)])
# Tree ADT
