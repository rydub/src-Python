...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 3B
        import copy
        temp = copy.copy(self.place)

        while temp.entrance != None and temp.entrance.name != 'Hive' and temp.bees == []:
            temp = temp.entrance
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        temp = copy.copy(self.place)
        i = 0
        while i < self.max_range and temp.entrance != None and temp.entrance.name != 'Hive' and temp.bees == []:
            temp = temp.entrance
            i += 1
        return random_or_none(temp.bees)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            backer = self.place.exit
            while backer:
                if backer.ant:
                    backer.ant.buff()
                    if backer.ant.container:
                        if backer.ant.ant:
                            backer.ant.ant.buff()
                backer = backer.exit
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        self.armor -= amount
        if self.true_queen and self.armor < 1:
            bees_win()
        if self.armor <= 0:
            self.place.remove_insect(self)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem 
    start_time = AntColony.time
    og_action, bee.action = bee.action, effect(bee.action)
    while AntColony.time < start_time + duration:
        None
    bee.action = og.action
