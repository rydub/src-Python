...
def find_centroid(cluster):
    ...
    longList = [restaurant_location(i)[1] for i in cluster]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(n) for n in cluster]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    r2s = [find_predictor(user, reviewed, i) for i in feature_fns]
    return max(r2s, key=lambda x:x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    ratings = {restaurant_name(i): predictor(i) for i in restaurants}
    ratings.update({restaurant_name(i): user_rating(user, restaurant_name(i))
    for i in reviewed})
    return ratings
    # END Question 9
